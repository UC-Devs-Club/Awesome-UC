...
**Windows**: Download and run the installer from [git-scm.com](https://git-scm.com/downloads). During setup, the default options are fine for beginners — just make sure "Git Bash" and "Add Git to PATH" are selected.
...
- **Communicate what you're working on**: A quick message in your club's Discord/Slack ("I'm working on the Linux workshop file") avoids two people editing the same section simultaneously.
...
A merge conflict happens when Git can't automatically reconcile changes — usually because two branches modified the **same lines** of the **same file** differently, and Git doesn't know which version is "correct."
...
5. Complete the merge with a commit:
   ...
   git commit
   ```
   (Git will pre-fill a merge commit message for you — usually fine to keep as-is)
...
- Most editors (including VS Code) show conflict markers with inline "Accept Current / Accept Incoming / Accept Both" buttons — far easier than resolving by hand in a text editor
